defmodule Sig.AccountsTest do
  use Sig.DataCase, async: true

  alias Sig.Accounts

  import Sig.AccountsFixtures
  alias Sig.Accounts.{User, UserToken}
  ...
  describe "change_user_registration/2" do
    test "returns a changeset" do
      ...
      assert changeset.required == [:password, :email, :individual_id, :org_id]
    end
  ...
  describe "change_user_password/2" do
    ...
    test "allows fields to be set" do
      ...
      assert changeset.valid?
      assert get_change(changeset, :password) == "N3w V@lid Password"
      assert is_nil(get_change(changeset, :hashed_password))
    end
  end
  ...
  describe "update_user_password/3" do
    ...
    test "updates the password", %{user: user} do
      {:ok, user} =
        Accounts.update_user_password(user, valid_user_password(), %{
          password: "N3w Valid P@ssword"
        })

      assert is_nil(user.password)
      assert Accounts.get_user_by_email_and_password(user.email, "N3w Valid P@ssword")
    end
  ...
  describe "reset_user_password/2" do
    ...
    test "updates the password", %{user: user} do
      {:ok, updated_user} = Accounts.reset_user_password(user, %{password: "n3w V@lid Password"})
    ...
    test "deletes all tokens for the given user", %{user: user} do
      _ = Accounts.generate_user_session_token(user)
      {:ok, _} = Accounts.reset_user_password(user, %{password: "n3w V@lid Password"})
      refute Repo.get_by(UserToken, user_id: user.id)
    end
  end
