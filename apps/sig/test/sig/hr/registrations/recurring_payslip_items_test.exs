defmodule Sig.HR.Registrations.RecurringPayslipItemsTest do
  ...
  describe "list_by/2" do
    ...
    test "lists recurring payslip items by category" do
      org = insert(:org)
    ...
    test "when category has no recurring payslip items" do
      ...
      assert RecurringPayslipItems.list_by(category) == []
    end
  end

  describe "count_by/1" do
    test "counts recurring payslip items by recurring item model" do
      org = insert(:org)
      rim = insert({:payslip_recurring_item_model, :fixed_amount}, org: org)
    ...
    test "when recurring item model has no recurring payslip items" do
      ...
      assert RecurringPayslipItems.count_by(rim) == 0
    end

    test "counts recurring payslip items by category" do
      org = insert(:org)
      category = insert(:payslip_category, org: org)
