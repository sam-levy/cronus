defmodule SigLive.Individuals.New do
  ...
  prop organization_id, :string, required: true
  ...
  def handle_event("fetch_individual_by_cpf", %{"validate_cpf" => %{"cpf" => cpf}}, socket) do
    case Entities.fetch_individual_by_cpf(socket.assigns.organization_id, cpf) do
      {:ok, individual} ->
  ...
  def handle_event("save", %{"individual" => individual_params}, socket) do
    ...
    case Entities.create_individual(organization_id, attrs) do
      {:ok, _individual} ->
        Entities.broadcast_organization_individuals(organization_id)
